...
create_CV_object <-  function(data_location,
  ...
  if(is_google_sheets_location){
    ...
    cv$references <- read_gsheet(sheet_id = 'reference')
  } else {
...
print_contact_info <- function(cv){
  glue::glue_data(
    cv$contact_info,
    "- <i class='{prefix} fa-{icon}'></i> {contact}"
  ) %>% print()
...
print_references <- function(cv,glue_template = "default"){
  if(glue_template == "default"){
    glue_template <- "
### {loc}
...
\n\n\n"
  ...
  section_data <- dplyr::filter(cv$references)
